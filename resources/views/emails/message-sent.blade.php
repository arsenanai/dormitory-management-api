...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>Hello {{ $recipient->first_name ? $recipient->first_name . ' ' . $recipient->last_name : $recipient->name }},</p>
            <p>You have received a new message from the administration.</p>
            <div class="message-box">
                <p><strong>{{ $message->title ?? 'Notification' }}</strong></p>
                <p>{{ \Illuminate\Support\Str::limit(strip_tags((string) $message->content), 500) }}</p>
            </div>
